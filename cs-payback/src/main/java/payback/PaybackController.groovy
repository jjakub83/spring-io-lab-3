...
class PaybackController {
    ...
    ResponseEntity<Void> register(@RequestBody Purchase purchase, UriComponentsBuilder uriBuilder) {
        ...
        return created(uriBuilder.path('/paybacks/{customerId}')
            .buildAndExpand(result.customerId).toUri()).build()
    }

    @RequestMapping(value = '/{customerId}', method = GET)
    List<Payback> paybacks(@PathVariable Long customerId) {
        return service.customerPaybacks(customerId)
    }

    @RequestMapping(value = '/{customerId}/summary', method = GET)
